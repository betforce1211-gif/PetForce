...
│       ├── Button.tsx           # Touch-optimized button (44dp+ touch targets)
│       ├── Input.tsx            # Form input with validation
│       └── Card.tsx             # Container component
├── features/
│   └── auth/
│       ├── components/          # Auth-specific components
│       │   ├── SSOButtons.tsx             # Google/Apple OAuth buttons
...
│           ├── VerifyEmailScreen.tsx      # Email verification success
│           ├── OAuthCallbackScreen.tsx    # OAuth deep link handler
│           ├── MagicLinkCallbackScreen.tsx # Magic link handler
│           └── DashboardScreen.tsx        # Main app (placeholder)
└── navigation/
...
The mobile app uses the shared `@petforce/auth` package for all authentication logic:

```typescript
import { useAuthStore, useAuth, useOAuth, useMagicLink } from '@petforce/auth';

// Auth state
const { isAuthenticated, user, isHydrated } = useAuthStore();

// Auth actions
const { loginWithPassword, registerWithPassword, logout } = useAuth();
const { loginWithGoogle, loginWithApple } = useOAuth();
const { sendLink, verifyLink } = useMagicLink();
```

### State Management

- **Zustand**: Global auth state (from @petforce/auth)
...
1. **Touch Targets**: Minimum 44dp (iOS) / 48dp (Android)
2. **Keyboard Handling**: KeyboardAvoidingView on all forms
3. **Safe Areas**: Automatic insets for notches/home indicators
4. **Platform Conventions**: Native navigation patterns
5. **Offline-First**: Ready for offline data sync
6. **Performance**: 60fps target, optimized renders
...
<Button
  ...
  size="lg"             // sm (36dp) | md (44dp) | lg (52dp)
  isLoading={loading}
  disabled={!isValid}
